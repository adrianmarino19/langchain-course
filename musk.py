...
if __name__ == '__main__':
    ...
    template = '''
    Given the information below from the person, please create:
    1. Summary.
    ...
    prompt = PromptTemplate(input_variables=['person'],
                            template=template)

    # llm = ChatOllama(model='gemma3:270m', temperature=0.9) ## Open weights, local LLM.
    llm = ChatOpenAI(model='gpt-3.5-turbo', temperature=0.9)

    chain = prompt | llm

    response = chain.invoke(person).content

    print(response)


# What about doing something more cryptic, like puttin the LinkedIn profiles of people for example? I think it'd be funny :).
# Langsmith implemented. Moving onto traditional machine learning my friend.
